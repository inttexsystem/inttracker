...
function makeSandbox(sources) {
  ...
  // SPECIALIZED-CONTROLS-B1: js/ui.js passou a ser o dono dos cinco primitivos
  // de controle especializado. Este harness stuba a camada compartilhada em vez
  // de carregar js/ui.js, entao os dois que a saida medida Manta usa sao
  // stubados aqui exatamente como textInput / selectInput acima. Ambos
  // reproduzem o contrato real de que as asserçoes dependem: o checkbox
  // colapsado continua sendo o DONO do estado lido por getPayload, e o switch
  // apenas o apresenta.
  sandbox.window.checkboxInput = (opts) => {
    const o = opts || {};
    const n = makeNode('input');
    n.setAttribute('type', 'checkbox');
    n.className = o.collapsed ? 'rv-checkbox-collapsed' : 'rv-checkbox';
    if (o.ariaLabel) n.setAttribute('aria-label', o.ariaLabel);
    if (o.disabled) n.setAttribute('disabled', 'disabled');
    if (typeof o.onchange === 'function') n.addEventListener('change', o.onchange);
    n.checked = !!o.checked;
    return n;
  };
  sandbox.window.switchToggle = (opts) => {
    const o = opts || {};
    const box = o.input || sandbox.window.checkboxInput({ collapsed: true, checked: o.checked });
    if (o.label && !box.getAttribute('aria-label')) box.setAttribute('aria-label', o.label);
    const track = sandbox.window.el('span', { class: 'rv-switch-track' },
      sandbox.window.el('span', { class: 'rv-switch-knob' }));
    const control = sandbox.window.el('label', { class: 'rv-switch' }, box, track);
    if (o.tone && o.tone !== 'brand') control.setAttribute('data-rv-switch-tone', o.tone);
    return sandbox.window.el('div', { class: 'rv-switch-field' },
      o.label ? sandbox.window.el('span', { class: 'rv-switch-label' }, o.label) : sandbox.window.el('span', {}),
      control);
  };
...
test('R3/19h. gates estruturais preservados (nenhum arquivo gated cresceu)', () => {
  ...
  // 2709 -> 2716: +7 linhas da passada 3 de altura, todas no bloco
  // normalizeMovementModalControls — o comentario que registra a divisao do
  // antigo MOVEMENT_CONTROL_HEIGHT generico em MOVEMENT_TEXTAREA_MIN_HEIGHT
  ...
  // 2870 -> 2975: +105 linhas de NATIVE-RECEIPT-COORDINATED-RELEASE-P2-C — o
  // bloco de TRANSICOES DE STATUS DO PEDIDO. Ele SUBSTITUIU o UPDATE direto em
  // `pedidos.status` pelos dois escritores canonicos do servidor
  // (alterar_status_pedido e cancelar_pedido, atras do portao de elegibilidade
  // D7), com submissao da revisao, estado de conflito explicito, motivo
...
// UI-CONSOLIDATION-PHASE-5-PASS-3-CONTROL-HEIGHT-R1-A1 fechou a propriedade de
// ALTURA GENERICA de controle. Ela alterou 15 assets de runtime: os dois donos
// compartilhados (js/ui.js, que passa a declarar a altura canonica dos campos
// de linha unica, e js/pedido-ui.js, cujo swatch passa a receber o tamanho por
// parametro) e 13 telas. A garantia original nao muda: um asset alterado e
// invalidado exatamente uma vez, sob um token declarado, e esse token nao vaza
// para nenhum asset que a passada 3 nao tenha realmente alterado.
const PASS3_TOKEN = '20260726-ui-p5-pass3-a1';
const PASS3_ASSETS = [
  'js/pedido-ui.js',
  'js/screens/admin-usuarios-modal.js',
  'js/screens/cadastros.js',
...
// UI-CONSOLIDATION-PHASE-5-PASS-4-SHADOW-R1-A1 fechou a propriedade de
// ELEVACAO. Ela alterou 6 assets de runtime: o dono compartilhado js/ui.js
// (toast e modal generico passam a declarar a elevacao canonica em vez de
// utilitarios Tailwind), os dois banners fixos, as duas acoes de importacao
// e a tela de login (declaracao malformada de duas camadas). A garantia
// original nao muda: um asset alterado e invalidado exatamente uma vez, sob
// um token declarado, e esse token nao vaza para nenhum asset que a passada 4
// nao tenha realmente alterado.
const PASS4_TOKEN = '20260726-ui-p5-pass4-shadow-a1';
const PASS4_ASSETS = [
...
// A passada 6 de tipografia (UIC-005) alterou 27 assets de runtime: a folha de
// tokens, os dois donos compartilhados (js/ui.js e js/document-links-surface-ui.js)
// e 24 telas. Ela e a ULTIMA passada a tocar cada um deles, entao todos saem dos
// conjuntos anteriores pelo mesmo mecanismo que as passadas 2-5 ja usaram.
const PASS6_TOKEN = '20260726-ui-p5-pass6-typography-r1';
const PASS6_ASSETS = [
  'css/tokens.css',
  'js/ui.js',
  'js/document-links-surface-ui.js',
...
// PASS-7-RUNTIME-ASSET-MANIFEST-EXTENSION-A2
// UI-CONSOLIDATION-PHASE-5-PASS-7-NATIVE-SELECT fechou a propriedade SELECT
// NATIVO (UIC-006). Ela e a primeira passada de consolidacao a ADICIONAR um
// asset de runtime: js/select-popover.js, o dono canonico do controle de
// escolha unica, carregado imediatamente ANTES de js/ui.js porque o adaptador
// selectInput() delega a ele. Alem disso retokenizou 11 assets existentes.
// Ela e a ULTIMA passada a toca-los, entao todos saem dos conjuntos anteriores
// pelo mesmo mecanismo que as passadas 2-6 ja usaram. A garantia original nao
// muda: um asset alterado e invalidado exatamente uma vez, sob um token
// declarado, e esse token nao vaza para nenhum asset que a passada 7 nao tenha
// realmente alterado.
const PASS7_TOKEN = '20260727-ui-p5-pass7-native-select-a1';
const PASS7_ADDED_ASSETS = ['js/select-popover.js'];
const PASS7_RETOKENED_ASSETS = [
  'css/tokens.css',
  'js/ui.js',
  'js/screens/admin-usuarios.js',
  'js/screens/pedido-itens-edit.js',
...
const PASS7_ASSETS = PASS7_ADDED_ASSETS.concat(PASS7_RETOKENED_ASSETS);

// A correcao A4 da passada 7 ligou o rotulo visivel de dezenove comboboxes ao
// seu trigger canonico e devolveu a geometria ratificada aos tres controles que
// styleSelect() de op-nova sobrescrevia. Ela alterou cinco telas — quatro delas
// entram no conjunto da passada 7 pela PRIMEIRA vez — e e a ULTIMA a toca-las.
const PASS7_A4_TOKEN = '20260727-ui-p5-pass7-native-select-a4-a11y-geometry';
const PASS7_A4_ASSETS = [
  'js/screens/admin-usuarios-modal.js',
  'js/screens/cadastros.js',
  'js/screens/expedicao-admin.js',
  'js/screens/op-nova.js',
...
// A correcao A5 removeu o chevron legado que wrapSelect() ainda desenhava por
// cima do trigger canonico em op-nova. Ela alterou UM asset, que ja pertencia
// ao conjunto da A4, e e a ULTIMA a toca-lo.
...
// SPECIALIZED-CONTROLS-B1 criou os cinco primitivos de controle especializado
// (textarea, checkbox, switch, range e conteudo visualmente oculto), moveu a
// geometria deles para css/tokens.css e migrou as 22 construcoes de runtime.
// Retokenizou exatamente os 18 assets que alterou. Um token B1 e estritamente
...
// PEDIDO-CLIENT-EDITOR-REQUEST-SUBMISSION-R1 acrescentou o editor do Cliente
// como um arquivo NOVO em index.html — nao um asset retokenizado, nao existia
// em NENHUM commit historico anterior. Entra na lista de adicoes declaradas
// pelo mesmo motivo e da mesma forma que os demais modulos que ordens
// anteriores acrescentaram: para que a comparacao posicional com 4532f76
// continue provando que nada foi removido ou reordenado, e para que os
// varredores "todo js/screens/* nao declarado tem de ser byte-identico a X"
// nao tentem comparar um arquivo que ainda nao existia naquele commit.
const CLIENT_EDITOR_ADDED_ASSETS = ['js/screens/cliente-pedido-edit.js'];

// INTTRACKER-PRODUCTION-CUTOVER-R1 promoveu ucrjtfswnfdlxwtmxnoo a producao
// definitiva e retirou gqmpsxkxynrjvidfmojk de todo papel de runtime. Alterou
// exatamente TRES assets, e nenhum deles pertence a qualquer tier anterior:
// sao o seam de configuracao e os dois modulos que consomem a permissao de
// escrita do ambiente. Por isso este e o PRIMEIRO tier cujos assets precisam
// ser subtraidos explicitamente das derivacoes _AINDA_EM_ e dos conjuntos
// `declarados` — os tiers de marca e de tela anteriores eram subconjuntos de
// B1_ASSETS e por isso ja vinham subtraidos de graca.
//
// A RETOKENIZACAO AQUI NAO E COSMETICA. Sem ela um browser com js/config.js em
// cache continuaria resolvendo para o projeto RETIRADO e escrevendo num banco
...
// NATIVE-RECEIPT-COORDINATED-RELEASE-P2 (A+B+C). A fase conectou o frontend
// aos escritores canonicos ja aplicados em producao: disponibilidade nativa,
// ajuste atomico, inicio de producao, fila de aceite do fornecedor, entrega
// Tapete atomica (TD3), recuperacao de acabamento, estorno e correcao de
// expedicao, status e cancelamento do Pedido, e o painel consolidado de
// producao. Dezesseis modulos mudaram de comportamento e um e novo; um cliente
// com qualquer um deles em cache operaria contra um contrato que nao existe
// mais, entao a retokenizacao e obrigatoria.
//
// Como toda ordem anterior, o tier declara o seu token e a sua lista, e as
// derivacoes _AINDA_EM_, os conjuntos `declarados` e a lista posicional de
// R3/20c2 subtraem esta lista.
const P2_TOKEN = '20260731-native-receipt-p2';
const P2_ASSETS = [
  'js/calculo-op.js',
  'js/delete-helpers.js',
  'js/supabase-client.js',
  'js/router.js',
  'js/screens/op-distribuicao-ui.js',
  'js/screens/op-nova.js',
  ...
  'js/screens/op-writes.js',
  'js/screens/pedido-detail-data.js',
  'js/screens/pedido-detail-events.js',
  'js/screens/entrega-form.js',
  'js/screens/entrega-writes.js',
  'js/screens/expedicao-admin.js',
  'js/screens/fornecedor.js',
  'js/screens/ordem-compra-receipt-events.js',
];
// Modulo NOVO da fase: entra em ADDED_SINCE_4532F76 pelo mesmo motivo que os
// anteriores — um asset acrescentado nao e um asset retokenizado.
const P2_ADDED_ASSETS = ['js/screens/pedido-producao-panel.js'];
...
const R3_ASSETS_AINDA_EM_R3 = R3_ASSETS
...
const ADDED_SINCE_4532F76 = PRIORITY_ADDED_ASSETS.concat(BATCH2_ADDED_ASSETS)
  ...
  .concat(ALTERACAO_REVIEW_ADDED_ASSETS).concat(P2_ADDED_ASSETS);
...
test('R3/20c6. o lote 3 nao retokenizou nenhum asset que nao alterou', () => {
  ...
  // NATIVE-RECEIPT-COORDINATED-RELEASE-P2 passou a ser a ULTIMA ordem a
  // alterar pedido-detail-data.js: `ajuste_revisao` e `revisao` entraram no
  // read model e a disponibilidade nativa por OP passou a ser carregada, ao
  // mesmo tempo em que o fallback plano de ordens de fio foi retirado. O
  // sujeito do guard nao muda: o lote 3 continua proibido de arrastar asset
  // algum, e cada asset segue verificado contra a ordem que o alterou POR
  // ULTIMO.
  const intocados = [
...
test('R3/20c2. todo asset nao relacionado conserva o token que tinha em 4532f76', () => {
  ...
  for (let i = 0; i < before.length; i++) {
    ...
    if (CUTOVER_ASSETS.includes(before[i].path)) continue;
    ...
    if (B1_ASSETS.includes(before[i].path)) continue;
    if (P2_ASSETS.includes(before[i].path)) continue;
...
test('A2/20c9. os assets da correcao A2 carregam exatamente o token declarado, e ele nao vaza', () => {
  // A passada 3 retokenizou parte deles; o token da A2 sobrevive apenas nos
  // demais, e PASS3/20c12 verifica os retokenizados literalmente.
  for (const rel of PASS2_A2_ASSETS_AINDA_EM_A2) {
  ...
  assert.deepEqual(carriers.sort(), PASS2_A2_ASSETS_AINDA_EM_A2.slice().sort(),
    'exatamente os assets da A2 que a passada 3 nao tocou podem carregar o token da A2');
  ...
  const declarados = new Set(PASS2_A2_ASSETS.concat(PASS3_ASSETS).concat(PASS4_ASSETS).concat(PASS5_ASSETS).concat(PASS6_ASSETS).concat(PASS7_ASSETS).concat(PASS7_A4_ASSETS).concat(PASS7_A5_ASSETS).concat(PASS8_ASSETS).concat(B1_ASSETS).concat(SCREEN_GROUP_2_ASSETS).concat(CUTOVER_ASSETS).concat(CLIENT_EDITOR_ADDED_ASSETS).concat(P2_ASSETS));
...
test('A3/20c10. os assets da correcao A3 carregam exatamente o token declarado, e ele nao vaza', () => {
  ...
  const declarados = new Set(PASS2_A3_ASSETS.concat(PASS2_A4_ASSETS).concat(PASS3_ASSETS).concat(PASS4_ASSETS).concat(PASS5_ASSETS).concat(PASS6_ASSETS).concat(PASS7_ASSETS).concat(PASS7_A4_ASSETS).concat(PASS7_A5_ASSETS).concat(PASS8_ASSETS).concat(B1_ASSETS).concat(SCREEN_GROUP_2_ASSETS).concat(CUTOVER_ASSETS).concat(CLIENT_EDITOR_ADDED_ASSETS).concat(P2_ASSETS));
  for (const ref of assetRefs(indexHtml)) {
...
test('A4/20c11. os assets da correcao A4 carregam exatamente o token declarado, e ele nao vaza', () => {
  // A passada 3 retokenizou js/pedido-ui.js e js/ui.js; o token da A4 sobrevive
  // apenas nos assets que ela nao tocou, e PASS3/20c12 verifica os demais.
  for (const rel of PASS2_A4_ASSETS_AINDA_EM_A4) {
  ...
  assert.deepEqual(carriers.sort(), PASS2_A4_ASSETS_AINDA_EM_A4.slice().sort(),
    'exatamente os assets da A4 que a passada 3 nao tocou podem carregar o token da A4');
  ...
  const declarados = new Set(PASS2_A4_ASSETS.concat(PASS3_ASSETS).concat(PASS4_ASSETS).concat(PASS5_ASSETS).concat(PASS6_ASSETS).concat(PASS7_ASSETS).concat(PASS7_A4_ASSETS).concat(PASS7_A5_ASSETS).concat(PASS8_ASSETS).concat(B1_ASSETS).concat(SCREEN_GROUP_2_ASSETS).concat(CUTOVER_ASSETS).concat(CLIENT_EDITOR_ADDED_ASSETS).concat(P2_ASSETS));
  for (const ref of assetRefs(indexHtml)) {
    if (/^https?:/.test(ref.path)) continue;
    if (!ref.path.endsWith('.js') || declarados.has(ref.path)) continue;
...
// A passada 3 declara os seus assets e o seu token; nenhum outro asset pode
// carrega-lo, e ele tem de diferir de todo token anterior. Sao os dois donos
// compartilhados de geometria (js/ui.js e js/pedido-ui.js) mais as 13 telas que
// a propriedade de altura generica realmente alterou.
test('PASS3/20c12. os assets da passada 3 de altura carregam exatamente o token declarado, e ele nao vaza', () => {
  // A passada 4 retokenizou js/ui.js; o token da passada 3 sobrevive apenas
  // nos assets que ela nao tocou, e PASS4/20c13 verifica os demais.
  for (const rel of PASS3_ASSETS_AINDA_EM_PASS3) {
    assert.equal(tokenFor(rel), PASS3_TOKEN,
      rel + ' deve carregar exatamente o token declarado da passada 3');
  }
  const carriers = assetRefs(indexHtml).filter((r) => r.token === PASS3_TOKEN).map((r) => r.path);
  assert.deepEqual(carriers.sort(), PASS3_ASSETS_AINDA_EM_PASS3.slice().sort(),
    'exatamente os assets da passada 3 que a passada 4 nao tocou podem carregar o token da passada 3');
  for (const anterior of [PASS2_A4_TOKEN, PASS2_A3_TOKEN, PASS2_A2_TOKEN, PASS2_TOKEN, PASS1_TOKEN,
    BATCH1_TOKEN, BATCH2_TOKEN, BATCH3_TOKEN, R2_TOKEN]) {
    assert.notEqual(PASS3_TOKEN, anterior,
      'reusar um token anterior nao invalidaria cache algum');
  }
  // Um asset retokenizado tem de ter mudado de fato desde o checkpoint publicado.
  for (const rel of PASS3_ASSETS) {
    const committed = execFileSync('git', ['rev-parse', '7577f93:' + rel], { cwd: ROOT, encoding: 'utf8' }).trim();
    const worktree = execFileSync('git', ['hash-object', '--', rel], { cwd: ROOT, encoding: 'utf8' }).trim();
  ...
  const declarados = new Set(PASS3_ASSETS.concat(PASS4_ASSETS).concat(PASS5_ASSETS).concat(PASS6_ASSETS).concat(PASS7_ASSETS).concat(PASS7_A4_ASSETS).concat(PASS7_A5_ASSETS).concat(PASS8_ASSETS).concat(B1_ASSETS).concat(SCREEN_GROUP_2_ASSETS).concat(CUTOVER_ASSETS).concat(CLIENT_EDITOR_ADDED_ASSETS).concat(P2_ASSETS));
  for (const ref of assetRefs(indexHtml)) {
    if (/^https?:/.test(ref.path)) continue;
    if (!ref.path.endsWith('.js') || declarados.has(ref.path)) continue;
    const committed = execFileSync('git', ['rev-parse', '7577f93:' + ref.path], { cwd: ROOT, encoding: 'utf8' }).trim();
    const worktree = execFileSync('git', ['hash-object', '--', ref.path], { cwd: ROOT, encoding: 'utf8' }).trim();
    assert.equal(worktree, committed, ref.path + ' nao foi declarado pela passada 3 e deve seguir byte-identico a 7577f93');
  }
});

// A passada 4 declara os seus assets e o seu token; nenhum outro asset pode
// carrega-lo, e ele tem de diferir de todo token anterior. Sao o dono
...
test('PASS4/20c13. os assets da passada 4 de elevacao carregam exatamente o token declarado, e ele nao vaza', () => {
  ...
  for (const rel of PASS4_ASSETS) {
    const committed = execFileSync('git', ['rev-parse', '42ce915:' + rel], { cwd: ROOT, encoding: 'utf8' }).trim();
    const worktree = execFileSync('git', ['hash-object', '--', rel], { cwd: ROOT, encoding: 'utf8' }).trim();
  ...
  const declarados = new Set(PASS4_ASSETS.concat(PASS5_ASSETS).concat(PASS6_ASSETS).concat(PASS7_ASSETS).concat(PASS7_A4_ASSETS).concat(PASS7_A5_ASSETS).concat(PASS8_ASSETS).concat(B1_ASSETS).concat(SCREEN_GROUP_2_ASSETS).concat(CUTOVER_ASSETS).concat(CLIENT_EDITOR_ADDED_ASSETS).concat(P2_ASSETS));
  for (const ref of assetRefs(indexHtml)) {
    if (/^https?:/.test(ref.path)) continue;
    if (!ref.path.endsWith('.js') || declarados.has(ref.path)) continue;
...
test('PASS5/20c14. os assets da passada 5 de alinhamento carregam exatamente o token declarado, e ele nao vaza', () => {
  for (const rel of PASS5_ASSETS_AINDA_EM_PASS5) {
  ...
  assert.deepEqual(carriers.sort(), PASS5_ASSETS_AINDA_EM_PASS5.slice().sort(),
    'exatamente os assets da passada 5 que a passada 6 nao tocou podem carregar o token da passada 5');
  ...
  const declarados = new Set([...PASS5_ASSETS, ...PASS6_ASSETS, ...PASS7_ASSETS, ...PASS7_A4_ASSETS, ...PASS7_A5_ASSETS, ...PASS8_ASSETS, ...B1_ASSETS, ...SCREEN_GROUP_2_ASSETS, ...CUTOVER_ASSETS, ...CLIENT_EDITOR_ADDED_ASSETS, ...P2_ASSETS]);
  for (const ref of assetRefs(indexHtml)) {
    if (/^https?:/.test(ref.path)) continue;
    if (!ref.path.endsWith('.js') || declarados.has(ref.path)) continue;
...
// A passada 6 de tipografia declara os seus assets e o seu token; nenhum outro
// asset pode carrega-lo, e ele tem de diferir de todo token anterior. Sao a
...
test('PASS6/20c15. os assets da passada 6 de tipografia carregam exatamente o token declarado, e ele nao vaza', () => {
  ...
  for (const anterior of [PASS5_TOKEN, PASS4_TOKEN, PASS3_TOKEN, PASS2_A4_TOKEN, PASS2_A3_TOKEN,
    ...
    assert.notEqual(PASS6_TOKEN, anterior,
      'reusar um token anterior nao invalidaria cache algum');
  }
  // Um asset retokenizado tem de ter mudado de fato desde o checkpoint publicado.
  for (const rel of PASS6_ASSETS) {
    const committed = execFileSync('git', ['rev-parse', '212972d:' + rel], { cwd: ROOT, encoding: 'utf8' }).trim();
    const worktree = execFileSync('git', ['hash-object', '--', rel], { cwd: ROOT, encoding: 'utf8' }).trim();
  ...
  const declarados = new Set([...PASS6_ASSETS, ...PASS6_A1_ASSETS, ...PASS7_ASSETS, ...PASS7_A4_ASSETS, ...PASS7_A5_ASSETS, ...PASS8_ASSETS, ...B1_ASSETS, ...SCREEN_GROUP_2_ASSETS, ...CUTOVER_ASSETS, ...CLIENT_EDITOR_ADDED_ASSETS, ...P2_ASSETS]);
  for (const ref of assetRefs(indexHtml)) {
    if (/^https?:/.test(ref.path)) continue;
    if (!ref.path.endsWith('.js') || declarados.has(ref.path)) continue;
    const committed = execFileSync('git', ['rev-parse', '212972d:' + ref.path], { cwd: ROOT, encoding: 'utf8' }).trim();
    const worktree = execFileSync('git', ['hash-object', '--', ref.path], { cwd: ROOT, encoding: 'utf8' }).trim();
    assert.equal(worktree, committed, ref.path + ' nao foi declarado pela passada 6 e deve seguir byte-identico a 212972d');
  }
  // A folha de tokens continua carregada antes da folha responsiva.
  const ordem = assetRefs(indexHtml).map((r) => r.path);
  assert.ok(ordem.indexOf('css/tokens.css') < ordem.indexOf('css/responsive.css'),
    'css/tokens.css tem de preceder css/responsive.css');
...
// INTTRACKER-PRODUCTION-CUTOVER-R1 — o tier de maior consequencia de cache de
// toda esta lista. Um browser que retenha o js/config.js anterior continua
// apontando para o projeto RETIRADO gqmpsxkxynrjvidfmojk: leria e ESCREVERIA
// num banco morto, silenciosamente, enquanto a aplicacao publicada acredita
// estar em ucrjtfswnfdlxwtmxnoo. Foi exatamente esse o defeito que a PASS-1
// sofreu com css/tokens.css servido sem versao. Por isso os tres assets do
// cutover sao verificados aqui de forma literal.
test('R3/20c11. os assets de INTTRACKER-PRODUCTION-CUTOVER carregam exatamente o token da ordem', () => {
  ...
  for (const anterior of [
    ...
    assert.notEqual(CUTOVER_TOKEN, anterior,
      'o token do cutover tem de diferir de todo token anterior');
    for (const rel of CUTOVER_ASSETS) {
      assert.notEqual(tokenFor(rel), anterior,
        rel + ' nao pode reter o token superseded ' + anterior);
    }
  }
  // Os tres assets tem de ter mudado de fato desde o checkpoint de entrada.
  for (const rel of CUTOVER_ASSETS) {
    const committed = execFileSync('git', ['rev-parse', '9af58ae:' + rel], { cwd: ROOT, encoding: 'utf8' }).trim();
    const worktree = execFileSync('git', ['hash-object', '--', rel], { cwd: ROOT, encoding: 'utf8' }).trim();
    assert.notEqual(worktree, committed, rel + ' foi retokenizado, entao tem de ter mudado desde 9af58ae');
  }
  // E o projeto retirado nao pode sobreviver em codigo ativo de nenhum dos tres.
  for (const rel of CUTOVER_ASSETS) {
...
test('R3/20c13. o asset da correcao do guard de RPC de leitura carrega exatamente o token da correcao', () => {
  // NATIVE-RECEIPT-COORDINATED-RELEASE-P2 ABSORVEU este tier: a mesma
  // js/supabase-client.js voltou a mudar (as quatro leituras nativas provadas
  // entraram na allowlist e o inventario declarativo das catorze RPCs do P2
  // foi acrescentado como METADADO, sem autoridade de execucao). Pelo mesmo
  // mecanismo que toda ordem posterior usou, o asset passa a carregar o token
  // da ordem que o alterou POR ULTIMO, e o token superseded nao pode sobrar
  // em asset nenhum.
  assert.deepEqual(READ_RPC_GUARD_ASSETS, ['js/supabase-client.js'],
    'a correcao do guard de leitura altera exatamente um asset');
  assert.equal(tokenFor('js/supabase-client.js'), P2_TOKEN,
    'js/supabase-client.js deve carregar o token da ordem que o alterou por ultimo');
  const carriers = assetRefs(indexHtml)
    .filter((r) => r.token === READ_RPC_GUARD_TOKEN)
    .map((r) => r.path);
  assert.deepEqual(carriers, [],
    'o token da correcao do guard de leitura foi superseded e nao pode sobrar em asset algum');
  for (const anterior of [
    R2_TOKEN, BATCH1_TOKEN, BATCH2_TOKEN, BATCH3_TOKEN,
    PASS1_TOKEN, PASS2_TOKEN, PASS2_A2_TOKEN, PASS2_A3_TOKEN, PASS2_A4_TOKEN,
    PASS3_TOKEN, PASS4_TOKEN, PASS5_TOKEN, PASS6_TOKEN, PASS6_A1_TOKEN,
    ...
    assert.notEqual(P2_TOKEN, anterior,
      'o token do P2 tem de diferir de todo token anterior');
