...
// artifact, ordered db/01..db/83 sequence, terminal two migrations with
...
function buildDeploymentManifest({ dbDir = DB_DIR, applicationArtifact = APPLICATION_ARTIFACT, expectedBranch = EXPECTED_BRANCH } = {}) {
  ...
  assert.equal(terminalTwo[0].filename, DB82_FILENAME);
  assert.equal(terminalTwo[1].filename, DB83_FILENAME);
...
test('deployment manifest resolves exactly db/01..db/83, contiguous and unique', () => {
...
test('db/82 and db/83 are the terminal two migrations', () => {
  const filenames = fs.readdirSync(DB_DIR);
  const entries = resolveMigrationManifest(filenames, { expectedTerminal: EXPECTED_TERMINAL });
  const [second82, first83] = entries.slice(-2);
  assert.equal(second82.filename, DB82_FILENAME);
  assert.equal(first83.filename, DB83_FILENAME);
});

test('the full deployment manifest builds against the real repository', () => {
  const manifest = buildDeploymentManifest();
  assert.equal(manifest.migrations.length, 83);
