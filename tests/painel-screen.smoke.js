...
test('5. index.html: ordem op-latex-admin.js → painel.js → jspdf → inline', () => {
  ...
  assert.ok(painelIdx < inlineIdx, 'painel.js deve vir antes do inline');
});
...
test('9. inline AINDA contém main', () => {
  const inline = extractInlineScript(indexSrc);
