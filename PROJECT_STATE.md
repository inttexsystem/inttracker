...
- **Fase ativa:** `G28-B6 — canonical document links — READY FOR ARCHITECT ACCEPTANCE` (não aceita/fechada). A verificação funcional foi executada diretamente em staging `ucrjtfswnfdlxwtmxnoo`: matriz RPC 20/20, propriedade dupla preservada, wrapper atômico com rollback PostgreSQL provado, reader/adapter verificados contra a projeção canônica e fallback `supabase_unavailable` exercitado no frontend local configurado para staging. Não houve correção técnica.
- **Próxima ação autorizável:** aceite arquitetural de G28-B6. `G28-B7` permanece não autorizado; sem push; produção proibida.
- **Contrato B6 pendente apenas de aceite:** Documento→Pedido 0..1; Documento→OP 0..N; revisão canônica tipada/versionada; `document_candidates.pedido_id` e `document_events.pedido_id` mantidos sob propriedade do Ingestor; `pedido_manual` permanece sugestão.
