...
- **Última fase aceita:** `G28-B7 — exibição nas superfícies — CLOSED / ACCEPTED_WITH_NONBLOCKING_REMOTE_SMOKE_DEBT` (aceite arquitetural explícito em 2026-07-14; não inferido da numeração). Commit parcial Pedido-detail `ed35f049397af4061ed6e8bb2d9ec3056c543724`; commit de conclusão (todas as superfícies) `9ef61e1896af631bc5aeeced4af93c77051f4de4`. Débito não bloqueante aceito: smoke autenticado das superfícies B7 (Pedido/OP/timeline/busca) em staging `ucrjtfswnfdlxwtmxnoo`. B6 permanece `CLOSED / ACCEPTED_WITH_NONBLOCKING_TEST_DEBT` (técnico `b2f180ed0e6f1c2ee6c02881d0199d1bfaf29366`). Produção `bhgifjrfagkzubpyqpew` não acessada.
- **Fase ativa:** `G28-B8 — correção, revogação, restauração e auditoria — IMPLEMENTED / TESTED (local) / READY FOR ARCHITECT ACCEPTANCE` (IAexec não auto-fecha). Escopo mínimo completo estendendo a fronteira de comando canônica (sem escritor concorrente): **correção** (substitui o conjunto completo de vínculos ativos → nova revisão; anterior revogada, não apagada; exige a revisão ativa esperada; idempotente; falha em estado stale/divergente; grava ator/timestamp/motivo); **revogação/unlink** (estado explícito vazio `pedido_id` NULL sem OPs; revisão anterior preservada; motivo+ator; sugestão do Ingestor e decisão nunca tocadas); **restauração** (copia o conjunto Pedido/OP normalizado de uma revisão histórica para uma NOVA revisão ativa, carimbando `restored_from_revision_id`; a linha histórica nunca é reativada nem mutada; revalida validade/compatibilidade atual fail-closed; concorrência otimista + idempotência); **auditoria** (trilha read-only append-only: versão, ativa/revogada, Pedido, OPs, ator, criado_em, revogado por/em, motivo, origem de restauração, command id; mais recente primeiro; detecção de anomalia de unicidade da revisão ativa; fail-closed quando o histórico está indisponível).
- **Schema/RPC (migration aditiva `db/52_document_link_correction_revocation_restoration.sql`, NÃO aplicada):** coluna aditiva `document_link_revisions.restored_from_revision_id` (FK tipada + índice parcial; sem backfill); evolução do escritor único `registrar_vinculos_documento` (DROP da assinatura de 5 args + recriação com `p_reason`/`p_restored_from_revision_id` DEFAULT NULL — motivo → `revocation_reason` da revisão superseded, proveniência carimbada na nova revisão; chamadas posicionais de 5 args do wrapper atômico B6 inalteradas); nova RPC `restaurar_vinculos_documento` (lê a origem read-only e delega ao escritor único, sem duplicar lógica de compatibilidade; rejeita `restore_source_not_found`/`restore_source_mismatch` e propaga os outcomes fail-closed do escritor). Aditiva: sem DROP TABLE, sem backfill, sem tocar candidates/events/decisions nem as RPCs de decisão B5/legada.
...
- **Verificação remota exigida (não executada — Supabase proibido para o agente local):** aplicar + verificar `db/52` em staging `ucrjtfswnfdlxwtmxnoo` (coluna/FK/índice, assinatura/grants do escritor evoluído, semântica da RPC de restauração, replay idempotente, concorrência otimista, restauração fail-closed de alvo invalidado) e render autenticado admin do modal de administração (histórico, correção, unlink, restauração, feedback stale/conflict). Nenhuma nova forma de query remota além da leitura aditiva de histórico.
...
- **Próxima ação autorizável:** aceite arquitetural de `G28-B8`. Nenhuma fase posterior a `G28-B8` autorizada; sem push; produção proibida.
...
- **OPEN_ARCHITECT_DECISIONS:** NONE.
- **Fases planejadas, não iniciadas (deferred):** G28-C (validação real em staging) e G28-D (publicação) — aguardam aceite de B8. Nenhuma fase posterior a B8 autorizada.
