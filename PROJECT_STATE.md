...
- **Fase ativa:** `G28-B8 — correção, revogação, restauração e auditoria — STAGING FUNCTIONALLY VERIFIED / READY FOR ARCHITECT ACCEPTANCE` (não `CLOSED` nem `ACCEPTED`). O escritor canônico único, o contrato Documento→Pedido `0..1` / Documento→OP `0..N`, a trilha append-only e a fronteira do Ingestor permanecem inalterados.
