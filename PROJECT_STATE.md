...
- **Última fase aceita:** `G28-B8 — CLOSED / ACCEPTED_WITH_NONBLOCKING_AUTHENTICATED_MODAL_SMOKE_DEBT` (decisão arquitetural explícita). Produção `bhgifjrfagkzubpyqpew` não acessada.
- **Fase ativa:** `G28-C — DATABASE/PROJECTION MATRIX 16/16 PASS / READY FOR ARCHITECT ACCEPTANCE` (não fechada nem aceita). Validação direta Hermes em `ucrjtfswnfdlxwtmxnoo`; sem defeito de produto, schema, RPC, migration ou arquitetura; cleanup `G28-C-VERIFY` zero.
...
- **Próxima ação autorizável:** aceite arquitetural de `G28-C` somente. `G28-D: NOT AUTHORIZED`; sem push; produção proibida. Débito: `AUTHENTICATED_BROWSER_SMOKE_BLOCKED_BY_TOOLING`.
...
- **OPEN_ARCHITECT_DECISIONS:** aceite de `G28-C` somente.
