...
-- Cascata controlada de teste incluindo expedicao.
--
-- Politica:
-- - Sem producao.
-- - Expedicao nao bloqueia em staging/teste nesta fase.
-- - Entrega, OP filha e expedicao de teste podem ser removidas em cascata.
...
DECLARE
  ...
  v_expedicao_movimento_ids BIGINT[] := '{}'::BIGINT[];
  v_expedicao_movimento_item_ids BIGINT[] := '{}'::BIGINT[];
  v_impacto JSONB;
  ...
  v_expedicao_movimentos BIGINT := 0;
  v_expedicao_movimento_itens BIGINT := 0;
  v_filhas BIGINT := 0;
  v_op_itens BIGINT := 0;
...
BEGIN
  ...
  SELECT COALESCE(array_agg(em.id ORDER BY em.id), '{}'::BIGINT[])
    INTO v_expedicao_movimento_ids
    FROM public.expedicao_movimentos em
   WHERE em.expedicao_id = ANY(v_expedicao_ids);

  SELECT COALESCE(array_agg(emi.id ORDER BY emi.id), '{}'::BIGINT[])
    INTO v_expedicao_movimento_item_ids
    FROM public.expedicao_movimento_itens emi
   WHERE emi.movimento_id = ANY(v_expedicao_movimento_ids)
      OR emi.expedicao_item_id = ANY(v_expedicao_item_ids);
  ...
  v_expedicao_movimentos := COALESCE(array_length(v_expedicao_movimento_ids, 1), 0);
  v_expedicao_movimento_itens := COALESCE(array_length(v_expedicao_movimento_item_ids, 1), 0);
  ...
  IF v_expedicoes > 0
     OR v_expedicao_itens > 0
     OR v_expedicao_movimentos > 0
     OR v_expedicao_movimento_itens > 0
     OR v_entregas > 0
  ...
  v_impacto := jsonb_build_object(
    ...
    'policy', 'OP excluida em teste pode remover cadeia produtiva completa, incluindo expedicao, sem alterar op_numeros.',
    ...
    'counts', jsonb_build_object(
      ...
      'expedicao_movimentos', v_expedicao_movimentos,
      'expedicao_movimento_itens', v_expedicao_movimento_itens,
      'ops_filhas', v_filhas,
      ...
      'cascade_includes_expedicao', (v_expedicoes + v_expedicao_itens + v_expedicao_movimentos + v_expedicao_movimento_itens) > 0
    ),
    'ids', jsonb_build_object(
      ...
      'expedicao_movimento_ids', to_jsonb(v_expedicao_movimento_ids),
      'expedicao_movimento_item_ids', to_jsonb(v_expedicao_movimento_item_ids)
    )
  );
...
BEGIN
  ...
  v_impacto := jsonb_build_object(
    ...
    'cascade_reason', v_cascade_reason,
    'confirmation_required', CASE WHEN v_cascade THEN 'EXCLUIR TUDO' WHEN v_requires THEN 'EXCLUIR' ELSE NULL END,
    'policy', 'Pedido excluido em teste pode remover cadeia produtiva completa, incluindo expedicao, sem alterar op_numeros.',
    'counts', jsonb_build_object(
...
DECLARE
  ...
  v_remaining_expedicao_item_ids BIGINT[] := '{}'::BIGINT[];
  v_remaining_expedicao_ids BIGINT[] := '{}'::BIGINT[];
  v_remaining_op_ids BIGINT[] := '{}'::BIGINT[];
  v_remaining_op_item_ids BIGINT[] := '{}'::BIGINT[];
  v_op_id BIGINT;
  v_deleted_expedicao_movimento_itens BIGINT := 0;
  v_deleted_expedicao_movimentos BIGINT := 0;
  v_deleted_expedicao_itens BIGINT := 0;
  v_deleted_expedicoes BIGINT := 0;
...
BEGIN
  ...
  SELECT ARRAY(SELECT jsonb_array_elements_text(v_diag #> '{impacto,ids,expedicao_ids}')::BIGINT) INTO v_expedicao_ids;
  SELECT ARRAY(SELECT jsonb_array_elements_text(v_diag #> '{impacto,ids,expedicao_item_ids}')::BIGINT) INTO v_expedicao_item_ids;
  SELECT ARRAY(SELECT jsonb_array_elements_text(v_diag #> '{impacto,ids,expedicao_movimento_ids}')::BIGINT) INTO v_expedicao_movimento_ids;

  PERFORM set_config('app.retificacao_autorizada', 'on', true);

  DELETE FROM public.expedicao_movimento_itens
   WHERE movimento_id = ANY(v_expedicao_movimento_ids)
      OR expedicao_item_id = ANY(v_expedicao_item_ids);
  GET DIAGNOSTICS v_deleted_expedicao_movimento_itens = ROW_COUNT;

  DELETE FROM public.expedicao_movimentos
   WHERE expedicao_id = ANY(v_expedicao_ids);
  GET DIAGNOSTICS v_deleted_expedicao_movimentos = ROW_COUNT;

  DELETE FROM public.expedicao_itens
   WHERE expedicao_id = ANY(v_expedicao_ids)
      OR op_item_id = ANY(v_target_op_itens);
  GET DIAGNOSTICS v_deleted_expedicao_itens = ROW_COUNT;

  DELETE FROM public.expedicoes
   WHERE id = ANY(v_expedicao_ids)
      OR op_latex_id = ANY(v_target_ops);
  GET DIAGNOSTICS v_deleted_expedicoes = ROW_COUNT;
  ...
  SELECT COALESCE(array_agg(ei.id ORDER BY ei.id), '{}'::BIGINT[])
    INTO v_remaining_expedicao_item_ids
    FROM public.expedicao_itens ei
   WHERE ei.expedicao_id = ANY(v_expedicao_ids)
      OR ei.op_item_id = ANY(v_target_op_itens);

  IF COALESCE(array_length(v_remaining_expedicao_item_ids, 1), 0) > 0 THEN
    RAISE EXCEPTION USING
      MESSAGE = 'Exclusao interrompida: ainda existem itens de expedicao vinculados a OPs alvo.',
      DETAIL = format('expedicao_item_ids=%s; expedicao_ids=%s; op_item_ids=%s', v_remaining_expedicao_item_ids, v_expedicao_ids, v_target_op_itens);
  END IF;

  SELECT COALESCE(array_agg(e.id ORDER BY e.id), '{}'::BIGINT[])
    INTO v_remaining_expedicao_ids
    FROM public.expedicoes e
   WHERE e.id = ANY(v_expedicao_ids)
      OR e.op_latex_id = ANY(v_target_ops);

  IF COALESCE(array_length(v_remaining_expedicao_ids, 1), 0) > 0 THEN
    RAISE EXCEPTION USING
      MESSAGE = 'Exclusao interrompida: ainda existem expedicoes vinculadas a OPs alvo.',
      DETAIL = format('expedicao_ids=%s; op_ids=%s', v_remaining_expedicao_ids, v_target_ops);
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'deleted', jsonb_build_object(
      ...
      'expedicoes', v_deleted_expedicoes,
      'expedicao_itens', v_deleted_expedicao_itens,
      'expedicao_movimentos', v_deleted_expedicao_movimentos,
      'expedicao_movimento_itens', v_deleted_expedicao_movimento_itens,
...
BEGIN
  ...
  IF COALESCE(array_length(v_remaining_expedicao_item_ids, 1), 0) > 0 THEN
    RAISE EXCEPTION USING
      MESSAGE = 'Exclusao interrompida: ainda existem itens de expedicao vinculados ao Pedido alvo.',
      DETAIL = format('expedicao_item_ids=%s; expedicao_ids=%s; op_item_ids=%s', v_remaining_expedicao_item_ids, v_expedicao_ids, v_target_op_itens);
  END IF;
  ...
  SELECT COALESCE(array_agg(e.id ORDER BY e.id), '{}'::BIGINT[])
    ...
    FROM public.expedicoes e
   WHERE e.id = ANY(v_expedicao_ids)
      OR e.pedido_id = p_pedido_id
      OR e.op_latex_id = ANY(v_target_ops);

  IF COALESCE(array_length(v_remaining_expedicao_ids, 1), 0) > 0 THEN
    RAISE EXCEPTION USING
      MESSAGE = 'Exclusao interrompida: ainda existem expedicoes vinculadas ao Pedido alvo.',
      DETAIL = format('expedicao_ids=%s; pedido_id=%s; op_ids=%s', v_remaining_expedicao_ids, p_pedido_id, v_target_ops);
  END IF;
...
COMMENT ON FUNCTION public.remover_pedido(UUID, TEXT) IS
  'Remove Pedido de teste incluindo expedicao, limpando expedicao/entrega antes de ops. Nao altera op_numeros.';
COMMENT ON FUNCTION public.remover_op(BIGINT, TEXT) IS
  'Remove OP de teste incluindo expedicao, limpando expedicao/entrega antes de ops. Nao altera op_numeros.';
