...
**Fase ativa:** `G28-C — DATABASE/PROJECTION MATRIX 16/16 PASS / READY FOR ARCHITECT ACCEPTANCE` (não `CLOSED` nem `ACCEPTED`).
**Última fase aceita:** `G28-B8 — CLOSED / ACCEPTED_WITH_NONBLOCKING_AUTHENTICATED_MODAL_SMOKE_DEBT`.
**Próxima ação autorizável:** aceite arquitetural de G28-C somente; G28-D permanece `NOT AUTHORIZED`.
...
**G28-C:** staging database/canonical projection validation `16/16 PASS`, `READY FOR ARCHITECT ACCEPTANCE`; browser autenticado permanece dívida de tooling. **G28-D:** `DEFERRED / NOT AUTHORIZED`.
