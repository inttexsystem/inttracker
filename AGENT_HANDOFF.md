...
- **Next authorizable action:** `UI-ACTION-BUTTON-MIGRATION-1` (phase `iii`, screen lot 1 — `pedidos-list.js` + `cliente-pedidos-list.js`) authorized in the same architect message as this closeout, in progress — see its own section once closed. Phase `iii` lot 2 (`ops-list.js`) and beyond remain `NOT AUTHORIZED`, pending their own orders.
